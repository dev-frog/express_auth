...
import { Entity, PrimaryGeneratedColumn, Index, Column, CreateDateColumn, UpdateDateColumn, DeleteDateColumn, PrimaryColumn } from 'typeorm'
...
export default class User{
    ...
    static async login(email: string, password: string): Promise<Result<any>> {
        ...
        if(!user.confirmed){
            return new Result<any>(new Error('User not confirmed'),400)
        }
